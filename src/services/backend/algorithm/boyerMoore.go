...
func boyerMooreAlgorithm(text, pattern string) int {
	...
	fmt.Println("t ", pattern[0], ", e ", pattern[1], ", s ", pattern[2], ", t ", pattern[3])
	badCharTable := [256]int{}
	for i := range badCharTable {
		badCharTable[i] = len(pattern)
		// fmt.Println(badCharTable[i], i)
	}
...
// 	// fmt.Println(boyerMooreAlgorithm("Apa yang sedang kamu lakukan disini!", "kamu"))
// }
